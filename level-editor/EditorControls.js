var EditorModel = Class.extend({
	...
	init : function(scene, camera) {
		...
		var parameters =  { height: '1000', width: '2000', tileSize: '100', color: "#ffff00", Team: "", Obstacle : "", PowerUp : "", groundtexture: "Supernova.jpg"};
		...
		var updateBoardCtl = {color: "#ffff00", onChange: function() {
		...
		var selectObstacleCtl = {color: "#ffff00", onChange: function() {
		...
		var selectPowerUpCtl = {color: "#ffff00", onChange: function() {
		...
		var selectTeamCtl = {color: "#ffff00", onChange: function() {
			scope.onTeamSelection(scope.currentSelectedTeamId);
		}};
		...
		var addTeamParams = {color: "#ffff00", onCreate: function() {
		...
		var selectgroundtextureCtl = {onChange: function() {
		...
		var cleargroundtextureCtl = {onChange: function() {
			scope.clearGround();
		}};
		...
		// Import and Export
		var exportCtl = {onChange: function() {
			scope.exportJson();
		}};
    ...
    loadGround: function(groundtexture) {
 		this.clearGround();
        var texture = THREE.ImageUtils.loadTexture('images/'+groundtexture);
	...
	drawGridSquares : function(width, height, size) {
        var tileFactory = new TileFactory(this);
        this.mapTileMeshes = new THREE.Object3D();
		...
		var material = new THREE.LineBasicMaterial( { color: 0x000000, opacity: 0.5, transparent: false } );
	...
	onTeamCreation: function(teamId) {
		...
		var new_unit_card = new UnitCard(soldier_unit_prototype, unitcardSize, '../assets/cardImages/soldier.png');
		var new_unit_card2 = new UnitCard(artillery_unit_prototype, unitcardSize, '../assets/cardImages/artillery.png');
		var new_unit_card3 = new UnitCard(flamethrower_unit_prototype, unitcardSize, '../assets/cardImages/flamethrower.png');
	...
	onObstacleCreation: function(obstacleName) {
		...
		var new_obstacle_card = new ObstacleCard(new_obstacle_prototype, 80, "../assets/cardImages/rock.png");
	...
	onPowerupCreation: function(obstacleName) {
		...
		var new_obstacle_prototype = new Obstacle(obstacleName, 0.0, this.map_tileSize);
		var obstacleCardSize = 80;
		var new_obstacle_card = new ObstacleCard(new_obstacle_prototype, 80, "../assets/cardImages/crate.png");
	...
	onDocumentMouseMove :function ( event ) {
		...
		// detecting intersects on screen manuelly
	...
	onDocumentKeyDown: function ( event ) {
		...
		switch( event.keyCode ) {
			...
			case 37: this.rotateMesh(rotateMe, -0.5 * Math.PI); break; //left
			case 38: this.rotateMesh(rotateMe, 1 * Math.PI); break; //up
			case 39: this.rotateMesh(rotateMe, 0.5 * Math.PI); break; //right
			case 40: this.rotateMesh(rotateMe, 0); break; //down
		}
	},
	...
	exportJson: function() {
		...
		var output = JSON.stringify({obj:JSON.stringify(exportJson)});
		console.log(output);
